...
class Analyzer:
	...
	def __init__(self, ws_path=[], gname=''):
		...
		fname = os.path.join(ws_path, TRAINED_MODELS_DIRNAME, (gname+GROUP_PARAMS_FNAME))
		...
		#### I prefer not to load the parameters for better memory performace. Until i need it. ####
		self.__grid_params = []
	
	def find_best_metrics(self, testsets_gname, number):
		
		results_gpath = os.path.join(self.__ws_path, RESULTS_DIRNAME, testsets_gname, self.__gname)
		# Checks for proper input
		if not os.path.exists(results_gpath):
			raise IOError("No such directory: '%s'"%results_gpath)
		
		for fpath in glob.glob(os.path.join(results_gpath,("*"+self.__gname+"*.txt"))):
			...
			fname = "BEST_#"+str(number)+"#_"+os.path.basename(fpath).split('.')[0]+".csv"
	...
	def construct_ROCs(self, const_params, testsets_gname):
		...
		results_path = os.path.join( self.__ws_path, RESULTS_DIRNAME, testsets_gname, self.__gname)
		...
		if len(const_params) != len(self.__params) -1 :
			raise ValueError('Wrong number of constant parameters. Current models parameters are: %s' % str(self.__params.keys()))
		...
		if len(des_param) != 1:
			raise ValueError('Wrong name(s) of constant parameter(s). Current models parameters are: %s' % str(self.__params.keys()))
		
		# check the validity of parameter's values
		for key in const_params.keys():
			if const_params[key] not in (self.__params[key]):
				raise ValueError("Wrong '%s' parameter value -> '%s'. Available values: %s" % (key,str(const_params[key]),str(self.__params[key])))
		...
		for fname in eval_files:
			...
			metrics = []
			for ind in Analyzer.__extract_params_id_with_pattern(par, const_params):
	...
	def evaluate_on_testsets(self, testsets_gname):
		...
		testsets_gpath = os.path.join(self.__ws_path, TEST_SETS_DIRNAME, testsets_gname)
		
		# Checks for proper input
		if not os.path.exists(testsets_gpath):
			raise IOError("No such file: '%s'"%testsets_gpath)
		
		# load grid params from text file
		free_grid_params = 0
		if len(self.__grid_params) == 0:                     # laod params in case the have not been loaded
		...
		for testset_id,testset_dirname in enumerate(os.listdir(testsets_gpath)):
			...
			path = os.path.join(self.__ws_path, TEST_SETS_DIRNAME, testsets_gname, testset_dirname)
			...
			# prepare for saving the results
			res_path = os.path.join(self.__ws_path, RESULTS_DIRNAME, testsets_gname, self.__gname)
			Analyzer.__create_dir(res_path) # create evaluations directory if not exists
			
			# create text file with grid_params and results 
			# have to open grid params and copy to new file the combinations
			fname = os.path.join(res_path,(testset_dirname+"_"+self.__gname+EVALUATIONS_FNAME))
			f= open(fname, 'w')
			
			# evaluate group on testset and Simultaneously write results to file
			for comb,metric in enumerate(self.__evaluate_group(testset, behset)):
				print "EVALUATE: %d of %d MODELS.  TESTSET %d of %d."%(comb+1,self.__group_size,testset_id+1,len(os.listdir(testsets_gpath)))
				line = str( [self.__grid_params[comb], metric] )
				
				f.write(str(comb)+'#'+line+"\n")
			f.close()
			
		# free grid_params in case we load them here
		if free_grid_params:
			self.__grid_params = []	
	...
	def __calc_best_metrics(self, metrics, num):
		...
		for met in metrics:
			...
			dists = np.append(dists, np.round(np.power((fpr*fpr + (1-dr)*(1-dr)),0.5),3) )
	...
	def __evaluate_group(self, testset, behset, testset_id = ''):
		...
		for comb,params in enumerate(self.__grid_params):
			...
			# predict testset's samples inside Evaluator object and evaluate the detector
	...
	def __load_grid_params(self):
		...
		fname = os.path.join(self.__gpath, (self.__gname+PARAMS_COMB_FNAME)) # grid params file name
		f = open(fname, 'r')
	...
	def create_models(ws_path, gname, FaultDetector, trainset_name, params):
		...
		# load trainset
		trainset_path = os.path.join(ws_path, TRAIN_SETS_DIRNAME, trainset_name, DATASET_FNAME)
		...
		for i,comb in enumerate(grid_params):
			...
			print "TRAINING %d of %d MODELS.   window=%d"%(train_id, exp_num, window)
